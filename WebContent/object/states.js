...
States.prototype = {
  ...
  refreshUndoRedoUI: function ()
  {
	  if (this.undos_.length && !this.firstState_) {
		  $('#undo').attr('src', 'ressources/mode/undo_on.png');
	  } else {
		  $('#undo').attr('src', 'ressources/mode/undo.png');
	  }
	  
	  if (this.redos_.length) {
		  $('#redo').attr('src', 'ressources/mode/redo_on.png');
	  } else {
		  $('#redo').attr('src', 'ressources/mode/redo.png');
	  }
  },
